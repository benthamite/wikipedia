...
(defun wikipedia-history--require-revision-at-point ()
  "Return the revision alist at point, or signal an error."
  (or (wikipedia-history--revision-at-point)
      (error "No revision at point")))

(defun wikipedia-history-view-revision ()
  "View the wikitext of the revision at point."
  (interactive)
  (let* ((rev (wikipedia-history--require-revision-at-point))
         (revid (alist-get 'revid rev))
         (content (wp--get-revision-content
...
(defun wikipedia-history-browse-revision ()
  ...
  (let* ((rev (wikipedia-history--require-revision-at-point))
         (revid (alist-get 'revid rev))
         (url (wikipedia--revision-url wikipedia-history--page-title revid)))
    (browse-url url)))
