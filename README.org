...
The development repository is on [[https://github.com/benthamite/wikipedia][GitHub]].
...
scored without pressing =R=.  Automatic refreshes skip scoring without
signaling an error while a review is already running.
...
([[#h:diff-display-options][Diff display]]).  Missing revisions are
fetched asynchronously, so the watchlist remains responsive while the
diff loads.  The command
...
placeholder in the meantime.  Revision fetches respect Wikimedia
rate-limit responses by retrying after the requested delay.
...
local SQLite database and restored when the watchlist is reopened.  The
completion message reports how many entries were processed, scored,
skipped, or failed.  Entries whose diffs cannot be fetched are reported
as failed rather than being silently skipped.  Rate-limited revision
fetches are retried after the delay requested by Wikimedia before they
...
- *Watchlist* / *History* (=h= prefix): watchlist operations (open,
  ...
  undo, restore).
- *Sync & Mirror* (=s= prefix): sync commands (sync page, sync
  watchlist, sync update, mirror).
- *AI* (=a= prefix): AI commands (detect AI, clear overlays, generate
  citation, generate edit summary, review watchlist).  This column is
  shown only when =pangram.el= or =gptel= is installed; individual
  items are further filtered by their specific library dependency.
